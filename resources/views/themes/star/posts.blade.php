                    ...
                    <div class="blog-latest-news style-two col-md-8 col-sm-12 col-xs-12">
                            ...
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <div class="item">
                                    <figure class="image-box">
                                        <img style="border-radius:5px;height:200px;width:100%; object-fit:cover" src="<?php echo ($post->thumbnail_path)? $post->thumbnail_path:"/themes/".config('theme.active')."/images/blog/4.png";?>" alt="Awesome Image">
                                        <div class="overlay">
                                            <div class="inner">  
                                                <ul class="social">
                                    ...
                                    <div style="height:150px;overflow: hidden;"><b>{{$post->title}}</b> {!!$post->content!!}</div>  
                                </div> 
                            </div>
                            <?php
                            }
                        ...
                        @if ($posts->hasPages())
                                ...
                                <ul class="page_pagination style-two">
                                    {{-- Previous --}}
                                    <li class="tran3s page-item {{ $posts->onFirstPage() ? 'disabled' : '' }}">
                                        <a class=" tran3s page-link" href="{{ $posts->previousPageUrl() }}"><i class="fa fa-angle-left" aria-hidden="true"></i></a>
                                    </li>
                                     {{-- Page Numbers --}}
                                        @foreach ($posts->links()->elements[0] ?? [] as $page => $url)
                                            <li class="tran3s page-item {{ $page == $posts->currentPage() ? 'active' : '' }}">
                                                <a class="tran3s {{ $page == $posts->currentPage() ? 'active' : '' }} page-link " href="{{ $url }}">{{ $page }}</a>
                                            </li>
                                        @endforeach
                                    {{-- Next --}}
                                    <li class="tran3s page-item {{ !$posts->hasMorePages() ? 'disabled' : '' }}">
                                        <a class=" tran3s page-link" href="{{ $posts->nextPageUrl() }}"><i class="fa fa-angle-right" aria-hidden="true"></i></a>
                                    </li>
                                </ul>
