 <?php
if(!isset($data['programs']))
{
   echo "Block này chưa được cấu hình";
return;
}
?>
 <section class="tf-section tf-courses">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <div class="title-heading st-3">
                                <div class="sub-heading clr-pri-3 f-mulish">
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                                        <g>
                                          <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                                        </g>
                                    </svg>
                                    <span class="inner-sub st-1">{{ $data['title'] }}</span>
                                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                                        <g>
                                          <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                                        </g>
                                    </svg>
                                </div>
                                <h2 class="title clr-pri-2">{{ $data['sub_title'] }}</h2>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="slider-courses">
                                <div class="themesflat-carousel clearfix" data-margin="30" data-item="3" data-item2="3" data-item3="2" data-item4="1" data-auto="false" data-nav="true" data-loop="true">
                                    <div class="owl-carousel owl-theme none">









                                        <?php
                            foreach($programs as $program)
                            {
                                if(in_array($program->id,$data['programs']))
                                {
                            ?>
                                        <div data-dot="" class="item-courses wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1300ms">
                                            <div class="box-feature">
                                                <img src="<?php echo $program->thumbnail_path; ?>" alt="Image">
                                            </div>
                                            <div class="box-content">
                                                <div class="box-wrap">
                                                    <h4 class="title"><a href="classe-details.html"><?php echo $program->name;?></a></h4>
                                                    <p class="sub f-mulish"><?php echo $program->name;?></p>
                                                </div>
                                                <ul>
                                                    <li><i class="far fa-user-graduate clr-pri-5"></i>( <?php echo ($program->age_from/12)?>-<?php echo ($program->age_to/12)?> tuổi)</li>
                                                    <li><i class="far fa-book clr-pri-6"></i><?php echo $program->class_count;?></li>
                                                    <li><i class="fal fa-usd-circle clr-pri-3"></i><?php echo $program->tuition;?></li>
                                                    <li><i class="far fa-clock clr-pri-8"></i>6 Months</li>
                                                </ul>
                                            </div>
                                        </div>
                                         <?php
                            }
                                }      
                            ?>




                                        










                                    </div>
                                </div><!--/.themesflat-carousel-->
                            </div>
                        </div>
                    </div>
                </div>
            </section>


