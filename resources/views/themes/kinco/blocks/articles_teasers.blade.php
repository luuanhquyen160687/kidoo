 <section class="tf-section tf-sc-blog">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
                            <div class="heading st-1">
                                <div class="title-heading">
                                    <div class="sub-heading clr-pri-3 f-mulish"><span class="inner-sub st-2">{{ $data['title'] }}</span>
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="77" height="30" viewBox="0 0 77 30">
                                            <g>
                                            <image width="77" height="30" xlink:href="data:img/png;base64,"/>
                                            </g>
                                        </svg>
                                    </div>
                                    <h2 class="title clr-pri-2">{{ $data['sub_title'] }}</h2>
                                </div>
                                <div class="heading-btn st-1">
                                    <a href="/tin-tuc" class="fl-btn st-3">
                                        <span class="inner">Tin tức</span>
                                    </a>
                                </div>
                            </div>
                        </div>







<?php foreach ($posts as $post)
                        {    
                        ?>
                        <div class="col-xl-4 col-lg-4 col-md-4 col-12">
                            <div class="box-artice fl-scale st-2 wow fadeIn animated" data-wow-delay="0.3ms" data-wow-duration="1200ms">
                                <div class="box-feature inner-scale"><a href="/tin-tuc-{{$post->slug}}">
                                    <img style="width: 100%; height: 250px; object-fit: cover;" src="/get_photo/{{$post->photo_id}}/500" alt="Image"></a></div>
                                <div class="box-content">
                                    <div class="meta-post st-1">
                                        <ul class="fx">
                                            <li><a href="blog-single.html" class="fx"><i class="far fa-calendar-alt clr-pri-3"></i>25 Dec 2021</a></li>
                                            <li><a href="blog-single.html" class="fx"><i class="far fa-comment-alt-dots clr-pri-3"></i>Comments (05)</a></li>
                                        </ul>
                                    </div>
                                    <h4 class="title-article-post"><a href="/tin-tuc-{{$post->slug}}">{{$post->title}}</a></h4>
                                    <div class="btn-rm">
                                        <a href="/tin-tuc-{{$post->slug}}" class="fl-btn st-4">
                                            <span class="inner">Chi tiết</span>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
<?php
                        }
                        ?>











                       
                    </div>
                </div>
            </section>
